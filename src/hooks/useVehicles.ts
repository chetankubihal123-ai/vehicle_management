  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.role === 'personal') {
        ...
        userTrips = mockTrips.filter(t => userVehicles.some(v => v.id === t.vehicle_id));
        userExpenses = mockExpenses.filter(e => userVehicles.some(v => v.id === e.vehicle_id));
  ...
  const addTrip = (trip: Omit<Trip, 'id' | 'created_at'>) => {
    ...
    setVehicles(prev => prev.map(v => 
      v.id === trip.vehicle_id 
        ? { ...v, current_mileage: trip.end_mileage }
        : v
    ));
  };
